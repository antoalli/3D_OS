...
"""
Paper: Regularization Strategy for Point Cloud via Rigidly Mixed Sample (CVPR 2021)
Impl. src: https://github.com/dogyoonlee/RSMix
"""
...
if __name__ == '__main__':
    import torch
    points = torch.rand(32,1024,3).numpy()
    labels = np.random.choice(points.shape[0], points.shape[0])
    mixup_points, lam, label, label_b = rsmix(points, labels, beta=1.0, n_sample=512, KNN=False)
    mixup_points = torch.as_tensor(mixup_points)
    print(f"mixup_points: {mixup_points.shape}")
    print(f"lam: {lam}")
    print(f"label: {label}")
